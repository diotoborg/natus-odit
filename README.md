# @diotoborg/natus-odit

![CI](https://github.com/diotoborg/natus-odit/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@diotoborg/natus-odit.svg?style=flat)](https://www.npmjs.com/package/@diotoborg/natus-odit)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)
[![NPM downloads](https://img.shields.io/npm/dm/@diotoborg/natus-odit.svg?style=flat)](https://www.npmjs.com/package/@diotoborg/natus-odit)


__@diotoborg/natus-odit__ is significantly faster than `JSON.stringify()` for small payloads.
...
@diotoborg/natus-odit requires a [JSON Schema Draft 7](https://json-schema.org/specification-links.html#draft-7) input to generate a fast `stringify` function.
...
@diotoborg/natus-odit array default x 6,243 ops/sec ±0.14% (98 runs sampled)
@diotoborg/natus-odit array json-stringify x 6,261 ops/sec ±0.30% (99 runs sampled)
...
@diotoborg/natus-odit large array default x 99.96 ops/sec ±0.22% (74 runs sampled)
@diotoborg/natus-odit large array json-stringify x 248 ops/sec ±0.07% (90 runs sampled)
...
@diotoborg/natus-odit long string x 15,979 ops/sec ±0.09% (96 runs sampled)
...
@diotoborg/natus-odit short string x 30,585,790 ops/sec ±0.27% (97 runs sampled)
...
@diotoborg/natus-odit obj x 6,866,434 ops/sec ±0.11% (100 runs sampled)
...
@diotoborg/natus-odit date format x 1,836,188 ops/sec ±0.12% (99 runs sampled)
...
<a name="example"></a>
Try it out on RunKit: <a href="https://runkit.com/npm/@diotoborg/natus-odit">https://runkit.com/npm/@diotoborg/natus-odit</a>
## Example

```js
const fastJson = require('@diotoborg/natus-odit')
...
Optionally, you may provide to `@diotoborg/natus-odit` an option object as second parameter:
...
```
If the object to stringify is missing the required field(s), `@diotoborg/natus-odit` will throw an error.

<a name="missingFields"></a>
#### Missing fields
If a field *is present* in the schema (and is not required) but it *is not present* in the object to stringify, `@diotoborg/natus-odit` will not write it in the final string.
Example:
```javascript
const stringify = fastJson({
  title: 'Example Schema',
...
<a name="defaults"></a>
#### Defaults
`@diotoborg/natus-odit` supports `default` jsonschema key in order to serialize a value
...
#### Pattern properties
`@diotoborg/natus-odit` supports pattern properties as defined by JSON schema.
...
`@diotoborg/natus-odit` supports additional properties as defined by JSON schema.
...
`@diotoborg/natus-odit` supports the **anyOf** and **oneOf** keywords as defined by JSON schema. Both must be an array of valid JSON schemas. The different schemas will be tested in the specified order. The more schemas `stringify` has to try before finding a match, the slower it will be.
...
#### If/then/else
`@diotoborg/natus-odit` supports `if/then/else` jsonschema feature. See [ajv documentation](https://ajv.js.org/keywords.html#ifthenelse).
...
```
If you need to use an external definition, you can pass it as an option to `@diotoborg/natus-odit`.
Example:
```javascript
const schema = {
  title: 'Example Schema',
...
`@diotoborg/natus-odit` to handle arrays starts increasing exponentially, leading
...
@diotoborg/natus-odit large array default x 48.72 ops/sec ±4.92% (48 runs sampled)
@diotoborg/natus-odit large array json-stringify x 157 ops/sec ±0.76% (86 runs sampled)
...
To achieve low cost and high performance redaction `@diotoborg/natus-odit`
...
Users are responsible for sending trusted data. `@diotoborg/natus-odit` guarantees that you will get
...
itself. You need to have `@diotoborg/natus-odit` installed for the standalone code to work.
